...
func NewBinaryLogStore(threshold uint64) (raft.LogStore, error) {
...
type safeMap struct {
	data map[uint64]*raft.Log
	mu   sync.RWMutex
}

func (sm *safeMap) Set(key uint64, value *raft.Log) {
	sm.mu.Lock()
	defer sm.mu.Unlock()
	sm.data[key] = value
}

func (sm *safeMap) Get(key uint64) (*raft.Log, bool) {
	sm.mu.RLock()
	defer sm.mu.RUnlock()
